...
async def get_token(acc, session):
    try:
        ...
        url = f"https://gettokenob53.vercel.app/token?uid={uid}&password={password}"
        async with session.get(url) as response:
            ...
            if response.status not in (200, 201):
                print(f"[get_token] ❌ HTTP Error for UID {uid}")
                return None
            ...
            # --- Hàm đệ quy tìm key "8" ---
            def find_token_in_key_8(obj, path="root"):
                if isinstance(obj, dict):
                    # Kiểm tra trực tiếp key "8"
                    if "8" in obj:
                        val = obj["8"]
                        # Nếu key "8" là string và có dữ liệu thì trả về luôn
                        if isinstance(val, str) and val:
                            return val, f"{path}.8"
                        # Nếu key "8" chứa object con, tiếp tục tìm sâu vào trong "8"
                        elif isinstance(val, (dict, list)):
                            res, p = find_token_in_key_8(val, f"{path}.8")
                            if res: return res, p
                    
                    # Nếu không thấy ở key "8" trực tiếp, tìm trong các key khác
                    for k, v in obj.items():
                        if k == "8": continue # Đã check ở trên
                        found, found_path = find_token_in_key_8(v, f"{path}.{k}")
                        if found:
                            return found, found_path
                            
                elif isinstance(obj, list):
                    for i, item in enumerate(obj):
                        found, found_path = find_token_in_key_8(item, f"{path}[{i}]")
                        if found:
                            return found, found_path
                return None, None

            token, token_path = find_token_in_key_8(data, "root")
            
            if token:
                print(f"[get_token] ✅ Token found in key '8' at {token_path}: {token[:50]}...")
                return token

            print(f"[get_token] ⚠️ Key '8' not found or empty for UID {uid}")
            return None
...
async def bounded_get_token(sem, acc, session):
    async with sem:
        await asyncio.sleep(2.0) # wait 2s between requests to avoid rate limit
        return await get_token(acc, session)
    ...
    try:
        ...
        if accounts:
            ...
            async with aiohttp.ClientSession(timeout=timeout) as session:
                ...
                tasks = [bounded_get_token(sem, f"{uid}:{password}", session) for uid, password in accounts]
...
async def refresh_token():
    ...
    try:
        ...
        async with aiohttp.ClientSession(timeout=timeout) as s:
            gringay = await get_token("4771727000:minhdev_BIGBULLCHEATS_KaTtYuhY", s)
    except Exception:
        pass
...
async def GetPlayerPersonalShow(payload, session, token):
    try:
...
async def sendLikes(uid):
    ...
    async with aiohttp.ClientSession(timeout=timeout) as session:
        InfoBefore = await GetPlayerPersonalShow(payload, session, _tokens[0])
        if not InfoBefore or "1" not in InfoBefore or "21" not in InfoBefore["1"]:
            return {"False": "Account does not exist"}, 200
        ...
        InfoAfter = await GetPlayerPersonalShow(payload, session, _tokens[0])
        ...
        if LikesAdded <= 0:
            return {"False": "Account Id '{}' with name '{}' has reached max likes today, try again tomorrow !".format(InfoBefore["1"]["1"], InfoBefore["1"]["3"])}, 200
